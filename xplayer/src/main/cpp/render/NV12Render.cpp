...
bool NV12Render::InitTexture(int width, int height) {
    //使用program
    glUseProgram(program);

    GLint yTexture = glGetUniformLocation(program, "yTexture");
    GLint uvTexture = glGetUniformLocation(program, "uvTexture");
    glUniform1i(yTexture, 0);
    glUniform1i(uvTexture, 1);
...
int NV12Render::getFragShader(char *fShader, int size) {
    return readAssetFile(mAssets, "fragNV12.frag", fShader, size);
}

int NV12Render::Render(unsigned char **data, int width, int height) {
    glUseProgram(program);
